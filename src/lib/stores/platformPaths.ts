...
export function getPlatformPaths(
  ...
  if (envPath) {
    ...
    // tears an absolute path in two at its drive letter: "C:\Toolscd\x"
    // becomes "C" and "\Toolscd\x", which resolves against the current
    // drive and silently yields a bogus directory alongside the real one.
